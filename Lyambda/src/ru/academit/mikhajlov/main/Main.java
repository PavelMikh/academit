...
public class Main {
    public static void main(String[] args) {
        ...
        Stream<String> stream = persons.stream().map(Person::getName).distinct();
        System.out.printf("Names: %s. ", stream.collect(Collectors.joining(", ")));
        System.out.println();

        System.out.printf("Average age of persons under 18 = %s.%n", persons.stream().filter(person -> person.getAge() < 18).mapToDouble(Person::getAge).average());

        //Map<K, V> K - names, V - averageAge
        Map<String, Double> map = persons.stream().collect(Collectors.groupingBy(Person::getName, Collectors.averagingDouble(Person::getAge)));
        map.forEach((name, age) -> System.out.printf("name %11s: average age = %s%n", name, age));

        System.out.println("List of people from 20 to 45 years sorted descending: " + persons.stream().filter(person -> person.getAge() >= 20 && person.getAge() <= 45)
